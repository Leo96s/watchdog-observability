...
<template>
  <div v-if="isOpen" @click="$emit('close')" class="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-black/60 backdrop-blur-md wd-overlay-in">
    <div @click.stop class="w-full max-w-[460px] rounded-[26px] border border-white/[.09] p-8 wd-modal-in"
      ...
      <p class="m-0 mb-5 text-[12px] text-[#6b7183] font-mono">@{{ service?.url }}</p>

      <div v-if="destinations.length" class="flex flex-col gap-2 mb-[18px]">
        <div v-for="dest in destinations" :key="dest.id" class="flex items-center justify-between gap-2.5 bg-[#0e131c] border border-white/[.06] rounded-[13px] px-3.5 py-2.5 wd-row-in">
          <span class="text-xs text-[#e8eaf0] truncate">
            <span class="text-[#60a5fa] font-extrabold uppercase text-[10px] mr-2">{{ dest.type }}</span>{{ dest.value }}
          </span>
          <button @click="removeDestination(dest.id)" class="text-[#6b7183] hover:text-[#f87171] shrink-0 flex transition-colors"><Trash2 :size="15" /></button>
        </div>
      </div>

      <div class="flex flex-col gap-3.5">
        <div>
          <label class="text-[10px] tracking-[.12em] uppercase text-[#7c8296] font-bold ml-1">Tipo de alerta</label>
          <div class="mt-[7px]">
            <BaseSelect v-model="type" :options="[{ label: 'Discord / Slack Webhook', value: 'webhook' }, { label: 'E-mail', value: 'email' }]" />
          </div>
        </div>
        <div>
          <label class="text-[10px] tracking-[.12em] uppercase text-[#7c8296] font-bold ml-1">Destino (URL ou email)</label>
