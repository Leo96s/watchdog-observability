...
import {
  ...
  Search, Plus, Check, X, Server, Gauge,
...
<template>
  ...
  <div class="flex h-screen overflow-hidden" style="background: radial-gradient(1200px 600px at 70% -10%, rgba(59,130,246,.10), transparent 60%), #0a0c11;">

    <!-- Sidebar -->
    <aside class="w-[72px] shrink-0 flex flex-col items-center py-[18px] gap-1.5 border-r border-white/[.06] bg-white/[.015]">
      ...
      <button title="Dashboard" @click="panel = null"
        ...
        :style="panel === null ? 'background:rgba(59,130,246,.14);color:#60a5fa' : 'color:#6b7183'">
        <LayoutGrid :size="20" />
      </button>
      <button :title="t.notifications" @click="openNotifications"
        ...
        :style="panel === 'notifications' ? 'background:rgba(59,130,246,.14);color:#60a5fa' : 'color:#6b7183'">
        <Bell :size="20" />
        <span v-if="events.unread.value > 0"
          ...
          style="box-shadow: 0 0 0 2px #0b0d12;">{{ events.unread.value > 9 ? '9+' : events.unread.value }}</span>
      </button>
      <button :title="t.historyNav" @click="panel = 'history'"
        class="w-11 h-11 rounded-[13px] flex items-center justify-center cursor-pointer transition-all hover:brightness-125"
        :style="panel === 'history' ? 'background:rgba(59,130,246,.14);color:#60a5fa' : 'color:#6b7183'">
        <Clock :size="20" />
      </button>
      <button :title="t.docs" @click="openDocs"
        class="w-11 h-11 rounded-[13px] flex items-center justify-center cursor-pointer transition-all text-[#6b7183] hover:bg-white/[.06] hover:text-[#e8eaf0]">
        <BookOpen :size="20" />
      </button>
      ...
      <button :title="t.settings" @click="panel = 'settings'"
        class="w-11 h-11 rounded-[13px] flex items-center justify-center cursor-pointer transition-all hover:brightness-125"
        :style="panel === 'settings' ? 'background:rgba(59,130,246,.14);color:#60a5fa' : 'color:#6b7183'">
    ...
    <main class="flex-1 overflow-y-auto overflow-x-hidden">
      <header class="sticky top-0 z-20 backdrop-blur-lg border-b border-white/[.06] px-[34px] py-5 flex items-center gap-5" style="background: rgba(10,12,17,.72);">
        <div class="flex flex-col gap-[3px]">
          ...
          <p class="m-0 text-[11px] tracking-[.16em] uppercase text-[#6b7183] font-semibold">{{ t.subtitle }}</p>
        </div>
        <div class="flex-1"></div>
        <div class="flex items-center gap-2.5 bg-[#141821] border border-white/[.07] rounded-xl px-3.5 py-2.5 w-[230px] focus-within:border-[#3b82f680] transition-colors">
          <Search :size="16" class="text-[#6b7183]" />
          <input v-model="search" :placeholder="t.search" class="bg-transparent border-none outline-none text-[#e8eaf0] text-[13px] w-full" />
        </div>
      ...
      <div class="px-[34px] pt-7 pb-12 max-w-[1400px] mx-auto">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-[30px]">
          <div class="rounded-2xl p-[18px_20px] border border-white/[.06]" style="background: linear-gradient(160deg,#151a24,#111520);">
            <div class="flex items-center justify-between">
              <span class="text-[11px] font-bold tracking-[.1em] uppercase text-[#7c8296]">{{ t.monitored }}</span>
            ...
            <div class="text-[12px] text-[#7c8296] mt-0.5">{{ t.monitoredSub }}</div>
          </div>
          <div class="rounded-2xl p-[18px_20px] border border-[#34d3991f]" style="background: linear-gradient(160deg,#131d1a,#101613);">
            <div class="flex items-center justify-between">
              <span class="text-[11px] font-bold tracking-[.1em] uppercase text-[#7c8296]">{{ t.operational }}</span>
            ...
            <div class="text-[12px] text-[#7c8296] mt-0.5">{{ t.operationalSub }}</div>
          </div>
          <div class="rounded-2xl p-[18px_20px] border border-[#f871711f]" style="background: linear-gradient(160deg,#1d1414,#161010);">
            <div class="flex items-center justify-between">
              <span class="text-[11px] font-bold tracking-[.1em] uppercase text-[#7c8296]">{{ t.failing }}</span>
            ...
            <div class="text-[12px] text-[#7c8296] mt-0.5">{{ t.failingSub }}</div>
          </div>
          <div class="rounded-2xl p-[18px_20px] border border-white/[.06]" style="background: linear-gradient(160deg,#151a24,#111520);">
            <div class="flex items-center justify-between">
              <span class="text-[11px] font-bold tracking-[.1em] uppercase text-[#7c8296]">{{ t.avgLatency }}</span>
              <span class="w-8 h-8 rounded-[10px] bg-[#3b82f624] flex items-center justify-center text-[#60a5fa]"><Gauge :size="17" /></span>
            </div>
            <div class="text-[34px] font-extrabold font-mono tracking-[-.03em] mt-3">{{ avgLatency ?? '—' }}<span v-if="avgLatency" class="text-base text-[#7c8296] font-semibold">ms</span></div>
            <div class="text-[12px] text-[#7c8296] mt-0.5">{{ t.avgLatencySub }}</div>
          </div>
        </div>
        ...
        <div class="flex items-center gap-3.5 mb-[18px]">
          ...
          <div class="flex gap-1.5 bg-[#111520] border border-white/[.06] rounded-[11px] p-1">
            <button v-for="f in ['all','up','down']" :key="f" @click="filter = f"
              ...
              :style="filter === f ? 'background:#3b82f6;color:#fff' : 'background:transparent;color:#8a90a0'">
              {{ f === 'all' ? t.all : f === 'up' ? 'Up' : 'Down' }}
              <span class="opacity-60">{{ f === 'all' ? services.length : f === 'up' ? upCount : downCount }}</span>
            </button>
          </div>
          <div class="flex-1"></div>
          <span class="text-[12px] text-[#6b7183] font-mono">{{ t.updates }}</span>
        </div>

        <!-- skeletons -->
        <div v-if="loading" class="grid gap-[18px]" style="grid-template-columns: repeat(auto-fill, minmax(370px, 1fr));">
          <div v-for="i in 6" :key="i" class="bg-[#141821] border border-white/[.06] rounded-[18px] p-[22px] h-[210px] overflow-hidden">
        ...
        <template v-else>
          ...
          <div v-else-if="services.length" class="text-center py-[70px] text-[#6b7183]">
            <div class="w-14 h-14 mx-auto mb-4 rounded-2xl bg-white/[.04] flex items-center justify-center text-[#4b5163]"><Search :size="26" /></div>
            <p class="m-0 text-sm font-semibold">{{ t.noResults }}</p>
          </div>
          <div v-else class="text-center py-20 px-5 rounded-2xl border-2 border-dashed border-white/[.08] bg-white/[.02]">
            <p class="text-[#6b7183] font-medium max-w-md mx-auto">{{ t.empty }}</p>
          </div>
        </template>
      </div>
    </main>
    ...
    <div v-if="panel === 'notifications' || panel === 'history' || panel === 'settings'"
      @click="panel = null"
      class="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/60 backdrop-blur-md wd-overlay-in">
      <div @click.stop class="w-full max-w-[520px] rounded-[26px] border border-white/[.09] p-8 wd-modal-in"
          ...
          <div class="flex items-center justify-between mb-1.5">
            ...
            <div class="flex items-center gap-2">
              <button v-if="events.notifs.value.length" @click="events.clearNotifs()"
                class="border-none rounded-lg bg-white/[.05] text-[#8a90a0] px-3 py-[7px] text-[11px] font-bold cursor-pointer transition-all hover:bg-[#f8717129] hover:text-[#f87171]">{{ t.clearAll }}</button>
              <button @click="panel = null" class="w-[34px] h-[34px] border-none rounded-[10px] bg-white/[.05] text-[#8a90a0] flex items-center justify-center cursor-pointer transition-all hover:bg-[#f8717129] hover:text-[#f87171]"><X :size="18" /></button>
            </div>
          </div>
          <div v-if="events.notifs.value.length" class="flex flex-col gap-2 mt-4 max-h-[400px] overflow-y-auto">
            <div v-for="(n, i) in events.notifs.value" :key="n.id" class="flex items-center gap-3 bg-[#0e131c] border border-white/[.06] rounded-[13px] px-3.5 py-3 wd-row-in" :style="`animation-delay:${Math.min(i,10)*40}ms`">
              ...
              <div class="min-w-0 flex-1">
                <div class="text-[13px] text-[#e8eaf0]"><span class="font-extrabold">{{ n.name }}</span> {{ evMeta(n.type).verb }}</div>
                <div class="text-[11px] text-[#6b7183] font-mono mt-0.5">@{{ n.url }}</div>
              </div>
              <span class="text-[11px] text-[#6b7183] font-mono shrink-0">{{ rel(n.ts) }}</span>
              <button @click="events.dismiss(n.id)" :title="t.remove" class="border-none bg-transparent text-[#4b5163] cursor-pointer shrink-0 flex p-0.5 transition-colors hover:text-[#f87171]"><X :size="15" /></button>
            </div>
          </div>
          <p v-else class="text-center text-[#6b7183] py-11 m-0 text-[13px]">{{ t.noEvents }}</p>
        </template>
        ...
        <template v-else-if="panel === 'history'">
          ...
          <div v-if="events.events.value.length" class="max-h-[400px] overflow-y-auto flex flex-col gap-2">
            <div v-for="(h, i) in events.events.value" :key="h.id" class="flex items-center gap-3 bg-[#0e131c] border border-white/[.06] rounded-[13px] px-3.5 py-3 wd-row-in" :style="`animation-delay:${Math.min(i,10)*40}ms`">
              ...
              <div class="min-w-0 flex-1">
                <div class="text-[13px] text-[#e8eaf0]"><span class="font-extrabold">{{ h.name }}</span> {{ evMeta(h.type).verb }}</div>
                <div class="text-[11px] text-[#6b7183] font-mono mt-0.5">{{ new Date(h.ts).toLocaleString(locale, { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }) }}</div>
              </div>
        ...
        <template v-else>
          ...
          <label class="text-[10px] tracking-[.12em] uppercase text-[#7c8296] font-bold ml-1">{{ t.language }}</label>
          <div class="flex flex-col gap-2 mt-2.5">
            <button v-for="l in LANGUAGES" :key="l.code" @click="setLang(l.code)"
              ...
              :style="lang === l.code ? 'background:rgba(59,130,246,.14);color:#60a5fa;border:1px solid rgba(59,130,246,.5)' : 'background:#0e131c;color:#e8eaf0;border:1px solid rgba(255,255,255,.08)'">
